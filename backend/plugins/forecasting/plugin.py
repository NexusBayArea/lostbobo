...
class ForecastingPlugin(BasePlugin):
    ...
    async def forecast(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if model == "weather":
            from backend.plugins.forecasting.models.weather.model import WeatherModel

            result = await WeatherModel().predict(model_payload)
        elif model == "ev":
            from backend.plugins.forecasting.models.ev.model import EVModel

            result = await EVModel().predict(model_payload)
        elif model == "market":
            from backend.plugins.forecasting.models.market.model import MarketModel
    ...
    async def capacity(self, payload: dict[str, Any]) -> dict[str, Any]:
        from backend.plugins.forecasting.forecasting import get_capacity_forecaster

        forecaster = get_capacity_forecaster()
        horizon = payload.get("horizon", "15m")
        from backend.plugins.forecasting.forecasting import ForecastHorizon
