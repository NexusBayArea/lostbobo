...
export const api = {
  ...
  async getControlRoomState(token: string): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/controlroom/state`, {
      headers: getHeaders(token)
    });
    if (!response.ok) {
      throw new Error('Failed to fetch control room state');
    }
    return response.json();
  },

  async executeControlCommand(token: string, action: string, runId: string): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/control/command?run_id=${runId}&action=${action}`, {
      method: 'POST',
      headers: getHeaders(token),
    });
    if (!response.ok) {
      throw new Error('Failed to execute control command');
    }
    return response.json();
  },
  ...
  async getSystemStatus(token?: string): Promise<any> {
  ...
  async getSystemJobLogs(token?: string): Promise<any[]> {
    const response = await fetch(`${API_BASE_URL}/system/job-logs`, {
      headers: getHeaders(token)
    });
    if (!response.ok) {
