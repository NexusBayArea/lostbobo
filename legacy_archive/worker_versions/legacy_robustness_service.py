...
class SimulationResult:
    ...
    telemetry: List[Dict[str, float]] = field(
        default_factory=list
    )  # Step size h, error, etc.
...
class AIReportInput:
    ...
    sobol_analysis: Dict[str, Dict[str, float]] = field(
...
class ParameterSampler:
    ...
    def generate_perturbations(
        self, parameters: List[ParameterConfig], method: SamplingMethod, num_runs: int
    ) -> List[Dict[str, float]]:
        """
        ...
        if not perturbable_params:
            raise ValueError(
                "Robustness analysis requires at least one perturbable parameter. No perturbable parameters found."
            )

        logger.info(f"Generating perturbations with seed: {self.seed}")

        if method == SamplingMethod.PERCENTAGE_5:
            return self._percentage_sampling(
                perturbable_params, parameters, 0.05, num_runs
            )
        elif method == SamplingMethod.PERCENTAGE_10:
            return self._percentage_sampling(
                perturbable_params, parameters, 0.10, num_runs
            )
        elif method == SamplingMethod.LATIN_HYPERCUBE:
            # Respect user percentage if available, fallback to 10%
            pct = 0.10
            return self._latin_hypercube_sampling(
                perturbable_params, parameters, num_runs, percentage=pct
            )
    ...
    def _sobol_sampling(
        ...
        bounds = [
            [
                p.min_value if p.min_value is not None else p.base_value * 0.8,
    ...
    def _latin_hypercube_sampling(
        ...
        for run_idx in range(num_runs):
            ...
            for param_idx, param in enumerate(perturbable_params):
                # Map [0, 1] to [1-pct, 1+pct] range
                sample = lhs_samples[run_idx, param_idx]
                factor = (1.0 - percentage) + sample * (2.0 * percentage)
...
class SimulationRunner:
    ...
    def __init__(self, random_seed: Optional[int] = None):
        ...
        self.rng = np.random.default_rng(random_seed or int(time.time()))
    ...
    async def run_simulation(
        self, parameters: Dict[str, float], run_id: int, timeout_sec: float = 300.0
    ) -> SimulationResult:
        """
        ...
        try:
            ...
            return await asyncio.wait_for(
                self._execute_physics_solve(parameters, run_id), timeout=timeout_sec
            )
    ...
    async def _execute_physics_solve(
        self, parameters: Dict[str, float], run_id: int
    ) -> SimulationResult:
...
class StatisticalAnalyzer:
    ...
    def compute_sensitivity_ranking(
        ...
        if non_convergent:
            logger.warning(
                f"Excluding {len(non_convergent)} non-convergent runs from sensitivity analysis"
            )
        ...
        if method == "sobol":
            ...
            for param_name, indices in sobol_results.items():
                sensitivities.append(
                    SensitivityMetric(
                        ...
                        influence_coefficient=indices[
                            "total_effect"
                        ],  # Use ST as primary ranker
            ...
            for param in parameters:
                ...
                param_values = [
                    r.parameters.get(param.name, param.base_value)
                ...
                sobol_idx = (
                    variance_contrib / total_variance if total_variance > 0 else 0.0
                )
                ...
                sensitivities.append(
                    SensitivityMetric(
                        ...
                        total_effect=float(
                            np.clip(sobol_idx * 1.1, 0, 1)
                        ),  # Mock interaction for non-sobol
    ...
    def _compute_variance_contribution(
        self, param_values: List[float], outputs: List[float]
    ) -> float:
    ...
    def compute_confidence_interval(
        self, results: List[SimulationResult], confidence: float = 0.95
        ...
        try:
            ...
            # Two-tailed t-distribution critical value
            alpha = 1.0 - confidence
            t_value = stats.t.ppf(1.0 - alpha / 2.0, df=n - 1)
        except ImportError:
            logger.warning("scipy not installed, using approximate t-value (2.0)")
...
class RobustnessService:
    ...
    def __init__(
        ...
        ] = {}  # Hash of baseline params -> result
    ...
    async def run_robustness_analysis(
        ...
        try:
            ...
            perturbations = sampler.generate_perturbations(
                config.parameters, config.sampling_method, config.num_runs
            )
            ...
            # Stable cache key using JSON sort_keys
            ...
            if baseline_key in self.baseline_cache:
                ...
                baseline_result = await self.runner.run_simulation(
                    baseline_params,
                    run_id=0,
                    timeout_sec=config.convergence_timeout_sec,
                )
    ...
    def create_ai_report_input(self, summary: RobustnessSummary) -> AIReportInput:
        """
    ...
    def create_ai_report_input_with_metadata(
        self, summary: RobustnessSummary, simulation_id: str = ""
    ) -> Dict[str, Any]:
        """
